...
public class AtraccionController {
	...
	public List<Atraccion> getAllAtracciones() {
		...
		if (atracciones.isEmpty()) {
			throw new ListaAtraccionesVaciaException("El listado de atraciones está vacío");
		}
	 ...
	 * @return ResponseEntity con la atracción si se encuentra, o no encontrado
	 *         (404) si no se encuentra.
	 */
	@GetMapping("/{id}")
	public ResponseEntity<Atraccion> getAtraccionById(@PathVariable Long id) {
		return atraccionService.findById(id).map(ResponseEntity::ok)
				.orElseThrow(() ->  new AtraccionNoEncontradaException("La atracción no existe" + id));
	}
	 ...
	 */
	@PostMapping
	public Atraccion createAtraccion(@Valid @RequestBody Atraccion atraccion) {
	 ...
	 * @return ResponseEntity con la atracción actualizada si se encuentra, o no
	 *         encontrado (404) si no se encuentra.
	 */
	@PutMapping("/{id}")
	public ResponseEntity<Atraccion> updateAtraccion(@Valid @PathVariable Long id,
			@RequestBody Atraccion atraccionDetails) {
	 ...
	 *         encontrado (404) si no se encuentra la atracción.
	 */
	@DeleteMapping("/{id}")
